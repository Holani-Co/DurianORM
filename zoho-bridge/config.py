...
# API names of the Deals fields for the customer's phone / city / email. Empty
# = not sent — a wrong/unset api_name would fail the whole create, so set these
# to the REAL names from Setup → Developer Space → API Names → Deals. For this
# org (confirmed 2026-07): ZOHO_CRM_MOBILE_FIELD=Mobile, ZOHO_CRM_CITY_FIELD=City,
# ZOHO_CRM_EMAIL_FIELD=Email. Phone + city come from the customer (deal-details
# gate); email is the inbox sender's address.
...
# Digital/Offline selector field. The Durian Home Studio Blueprints use an
# empty/non-empty criterion on "Integration Source": digital must write a
# valid non-empty source; offline must leave the field empty. An empty OFFLINE
# value therefore means "omit this field" (not the literal string "Offline").
# An empty selector field is fail-safe: the bridge still records the intended
# mode in Chatwoot and the Deal description, but does not risk INVALID_DATA
# from an unverified CRM API name.
...
ZOHO_CRM_BLUEPRINT_DIGITAL_VALUE = os.environ.get(
    "ZOHO_CRM_BLUEPRINT_DIGITAL_VALUE", "DurianORM"
)
ZOHO_CRM_BLUEPRINT_OFFLINE_VALUE = os.environ.get(
    "ZOHO_CRM_BLUEPRINT_OFFLINE_VALUE", ""
)
...
REVIEWS_AUTO_REPLY_MIN_STARS  = int(os.environ.get("REVIEWS_AUTO_REPLY_MIN_STARS", "1"))
# Zero-touch review policy: reviews of ALL ratings (>= MIN_STARS, now 1) auto-
# reply with their rating-appropriate template and resolve, UNLESS the template
# call flags the content needs_human — really bad content (legal/suing, fraud,
# abuse/defamation, safety hazard) — which is held as a card in the reviews
# inbox for a person. The needs_human judgment rides on the single template-pick
# call (no separate classifier), so there is no extra AI cost per review.
...
# additional_attributes.caption) and post a brand-safe public reply that names
# the product and invites the customer to DM — where the enquiry flow captures
# their contact + location. Public-safe: never posts price or stock. Dark-launched.
