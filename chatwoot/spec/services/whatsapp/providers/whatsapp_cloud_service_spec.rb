  ...
  describe '#sync_templates' do
    context 'when called' do
      it 'updated the message templates' do
        body_component = [{ type: 'BODY', text: 'Hello' }]
        stub_request(:get, 'https://graph.facebook.com/v22.0/123456789/message_templates')
          .to_return(
            { status: 200, headers: response_headers,
              body: { data: [
                { id: '1', name: 'test_template', language: 'en_US', category: 'MARKETING', status: 'APPROVED', components: body_component }
              ], paging: { next: 'https://graph.facebook.com/v22.0/123456789/message_templates' } }.to_json },
            { status: 200, headers: response_headers,
              body: { data: [
                { id: '2', name: 'next_template', language: 'en_US', category: 'UTILITY', status: 'APPROVED', components: body_component }
              ], paging: { next: 'https://graph.facebook.com/v22.0/123456789/message_templates' } }.to_json },
            { status: 200, headers: response_headers,
              body: { data: [
                { id: '3', name: 'last_template', language: 'en_US', category: 'MARKETING', status: 'PENDING', components: body_component }
        ...
        expect { subject.sync_templates }.to change(WhatsappTemplate, :count).by(3)
        expect(whatsapp_channel.reload.message_templates.pluck('name')).to eq(%w[test_template next_template last_template])
        expect(whatsapp_channel.reload.message_templates_last_updated).not_to eq(timstamp)
      end

      it 'keeps the previous cache unchanged when template sync fails' do
        stub_request(:get, 'https://graph.facebook.com/v22.0/123456789/message_templates')
          .to_return(status: 401)

        timstamp = whatsapp_channel.reload.message_templates_last_updated
        expect { subject.sync_templates }.to raise_error(Whatsapp::TemplateManagementService::Error)
